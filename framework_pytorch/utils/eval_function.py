...
def true_positive(pred, gt, axis=None):
    return np.sum((pred & gt), axis)

def true_negative(pred, gt, axis=None):
    return np.sum(((~pred) & (~gt)), axis)

def false_positive(pred, gt, axis=None):
    return np.sum((pred & (~gt)), axis)

def false_negative(pred, gt, axis=None):
    return np.sum(((~pred) & gt), axis)
...
def accuracy(pred, gt, axis=None):
    """ equal(pred, gt) / all(pred, gt)
        (tp + tn) / (tp + tn + fp + fn)
    """

    return np.mean((pred == gt), axis)
