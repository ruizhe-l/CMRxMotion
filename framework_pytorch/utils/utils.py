...
def dict_mean(eval_dict, axis=0):
    ...
    for key in eval_dict:
        ...
        if value.size == 2 or value.ndim == 2: # calculate mean along batch
...
def combine_2d_imgs_from_tensor(img_list, nmin=None, nmax=None, channel_first=False):
    if channel_first:
        img_list = [x.transpose(0, 2, 3, 1) for x in img_list if len(x.shape) == 4]
    imgs = []
    combined = None
    for im in img_list:
...
def combine_3d_imgs_from_tensor(img_list, vis_slices, nmin=None, nmax=None, channel_first=False):
    ...
    if channel_first:
        img_list = [x.transpose(0, 2, 3, 4, 1) for x in img_list if len(x.shape) == 5]
    tmp = img_list[0]
    vis_slices = min(vis_slices, tmp.shape[-2])
    idx = [int(tmp.shape[-2] // vis_slices * (s + 0.5)) for s in range(vis_slices)]
    imgs = None
    for n in range(tmp.shape[0]):
        ...
        for xs in img_list:
            ...
            tmp_xs = np.array(xs)
            tmp_list.append(tmp_xs[n, ..., idx, 0]) if len(tmp_xs.shape) == 5 else tmp_list.append(tmp_xs[n, ..., idx])
        img = combine_2d_imgs_from_tensor(tmp_list, nmin=nmin, nmax=nmax, channel_first=False)
